...
\texorpdfstring{$\operatorname{Def}_X$}{Def X}}
...
\begin{definition}
\label{definition-deformation-over-artin-ring}
...
$$
\iota:\mathcal{X}_A\times_{\operatorname{Spec}A}
...
\left\{
\begin{array}{c}
\text{deformations of $X$ over $A$}
\end{array}
\right\}\big/\text{isomorphism}.
\end{aligned}
$$
...
\noindent
More honestly,
$\operatorname{Def}_X(A)$ should be the
\emph{groupoid} of deformations and their
isomorphisms, rather than merely the set of
isomorphism classes. Passing to a set forgets
the automorphisms of each deformation. Those
automorphisms are one of the principal
reasons why $\operatorname{Def}_X$ need not
be representable.
...
$X\subset\mathbb{P}^7$. It therefore belongs
first to the local Hilbert problem, although
one can subsequently forget the embedding
and obtain an abstract deformation.

\subsection{Universal and miniversal formal
deformations}
\label{subsection-miniversal-deformations}
...
defines a functor
$$
h_R(A)=\operatorname{Hom}_{\text{local}
k\text{-alg}}(R,A).
$$
...
smoothness means that deformations can be
lifted through small extensions whenever
the compatible lifting problem has a
solution in $\operatorname{Def}_X$.
...
\boxed{
\operatorname{Def}_X
\text{ is the functor, whereas }
\operatorname{Spf}R
\text{ is a miniversal space for it.}}
$$
...
it is prorepresentable. Proper schemes over
a field satisfy the finiteness conditions
which usually ensure the existence of a
miniversal formal deformation. Infinitesimal
automorphisms are the typical obstruction to
upgrading it to a universal one.

For sufficiently well-behaved schemes the
tangent and obstruction spaces can be written
in terms of the cotangent complex:
$$
...
(\mathbb{L}_{X/k},\mathcal{O}_X).
$$
Morally, a miniversal base has the form
$$
\operatorname{Spf}
\frac{k[[t_1,\ldots,t_r]]}
{\text{obstruction equations}},
\qquad r=\dim_kT^1_X.
$$
The equations of the base record which
first-order directions survive the
obstructions in $T^2_X$.
...
construction seeks such an arc whose generic
fiber is smooth. It does not construct the
whole miniversal deformation space.
