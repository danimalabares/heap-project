...
def getNeighbours(tag):
  ...
  try:
    if tag.type in headings:
      left = Tag.get(Tag.ref == left, Tag.type == tag.type, Tag.active == True) # to deal with parts appropriately we select for part / chapter with the appropriate number
    else:
      left = Tag.get(Tag.ref == left, Tag.type != "item", Tag.active == True)
  ...
  try:
    if tag.type in headings:
      right = Tag.get(Tag.ref == right, Tag.type == tag.type, Tag.active == True) # to deal with parts appropriately we select for part / chapter with the appropriate number
    else:
      right = Tag.get(Tag.ref == right, Tag.type != "item", Tag.active == True)
  except Tag.DoesNotExist:
    right = None
  ...
  try:
    if tag.type in headings: # for now we don't deal with this, Kerodon might need this though
      ...
      up = Tag.get(Tag.ref == up, Tag.type != "item", Tag.active == True)
  except Tag.DoesNotExist:
    up = None
...
def show_tag(tag):
  ...
  try:
    tag = (Tag.select(Tag,
                      ...
                      History.html,
                      Reference.html)
                .where(Tag.tag == tag.upper(), Tag.active == True)
    ...
    try:
      inactive = Tag.get(Tag.tag == tag.upper(), Tag.active == False)
      replacement = Tag.get(
          Tag.ref == inactive.ref,
          Tag.type == inactive.type,
          Tag.active == True)
  ...
  if tag.type in headings:
    ...
    if headings.index(tag.type) >= headings.index(gerby.configuration.UNIT):
      tags = Tag.select().where(Tag.ref.startswith(tag.ref + "."), Tag.type << headings, Tag.active == True)
  ...
  if tag.type in headings and headings.index(tag.type) < headings.index(gerby.configuration.UNIT):
    ...
    if tag.type == "part":
      ...
      sections = Tag.select().where(Tag.type == "section", Tag.active == True)
