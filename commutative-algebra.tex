...
\begin{remark}
\label{remark-alternative-approach}
...
\begin{proof}
By contrapositive, we prove that if
$R_1$ is not finitely generated,
then $R_1 \otimes_{\mathbb{C}}R_2$
is not finitely generated.
It's clear that
$$
R_1 \otimes 1
=
\{r_1 \otimes 1:r_1 \in R_1\}
\subset R_1 \otimes_{\mathbb{C}}R_2
$$
is isomorphic to $R_1$.
But this is a subring of a finitely
generated ring, and thus
must be finitely generated too. This is
wrong, $\mathbb{C}[x,xy,xy^2,…]$
is an infinitely-generated subring of
$\mathbb{C}[x,y]$.

The idea then was to use the universal
property of tensor product. Consider
$$
\xymatrix{
R_1 \times R_2 \ar[r]^\mu
\ar[dr]_{\operatorname{id}\otimes \lambda}
&  R_1
\otimes_\mathbb{C}R_2\ar[d]^{\exists \psi}\\
R_1
}
$$
where $\lambda$ is a linear
functional on $R_2$ (which exists by 
Zorn's lemma since $R_2$ is some infinite
dimensional $\mathbb{C}$-vector space).
Note that $\operatorname{id} \otimes
\lambda(r_1,1)=r_1$.

Now suppose that $z_j$ are the finitely-many
generators of $R_1 \otimes R_2$
and each of them is of the form
$\sum_{k} a_{jk} \otimes b_{jk}$ for
$a_{jk} \in R_1, b_{jk} \in R_2$.
Then
$ r_1 \otimes 1
=\sum_{j,k} 
c_j (a_{jk} \otimes b_{jk})^{\alpha_j}$
for some $c_j \in \mathbb{C}$
and $\alpha_j \in \mathbb{Z}$.
Moreover
$$
r_1=\psi\mu(r_1,1)
=\alpha_j \sum \psi((a_{jk} \otimes
b_{jk})^{\alpha_j}),
$$
but this doesn't work since there
can be infinitely many $\alpha_j$.
