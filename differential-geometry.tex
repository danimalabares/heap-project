...
differentiable
map $N:S \to \mathbb{R}^3$ such that $N(p)
\perp T_pS$
...
Seja \(F:\mathbb{R}^3 \to \mathbb{R}^4\) dada
por
\[F(x,y,z)=(x^2-y^2,xy,xz,yz),\qquad (x,y,z)
= p \in \mathbb{R}^3.\]
...
aplicação \(\tilde{\varphi}:\mathbb{R}P^2 \to
\mathbb{R}^4\) dada por
...
\item \(\tilde{\varphi}\) é biunívoca; junto
com (a) e a compacidade de
\(\mathbb{R}P^2\), isto implica que \(\tilde{
\varphi}\) é um mergulho.
\end{enumerate}
\end{exercise}
...
I.e.,
	...
	logo
	\[x_2^2-y_2^2=x_1^2-y_1^2=\left(\frac{z_2}
	{z_1}\right)^2(x_2^2-y_2^2)\implies
...
\tilde{M}\) uma subvariedade mergulhada e \(X
\in \mathfrak{X}(M)\). Mostre que existe um
aberto \(U \subset \tilde{M}\) contendo \(M\)
e um campo \(\tilde{X} \in \mathfrak{X}(U)\)
tal que \(\tilde{X}|_{M}=X\). Caso \(M\) seja
subconjunto fechado de \(\tilde{M}\), prove
que \(U\) pode ser tomado igual a
\(\tilde{M}\). Se \(M\) não é subconjunto
fechado de \(\tilde{M}\), pode não existir
extensão de \(X\) definida em todo
\(\tilde{M}\).
\end{exercise}
...
\begin{align*}
\left<U,V\right>&=
\left<dR_{\text{exp}(tX)}\circ
dL_{\text{exp}(tX)^{-1}}U,
dR_{\text{exp}(tX)}\circ
dL_{\text{exp}(tX)^{-1}}V\right>\\
...
\left<\nabla f,X\right>=Xf, \qquad \forall
X\in\mathfrak{X}(M).
\end{equation}
\end{definition}
...
=\sum_{i}\left<E_iX^jE_j,E_i\right>+
X^j\left<\nabla_{E_i}E_j,E_i\right>\\
&=\sum_i
\left<E_iX^jE_j,E_i\right>=
\sum_iE_iX^j\left<E_j,E_i\right>=E_iX^i
\end{align*}
...
\begin{align*}
2\left<\nabla_XY,Z\right>
&=X\left<Y,Z\right>+Y\left<X,Z\right>-
Z\left<X,Y\right>\\
&-\left<X,[Y,Z]\right>+\left<Y,[Z,X]\right>+
\left<Z,[X,Y]\right>.
\end{align*}
\end{proof}
...
\item We compute $\left<\alpha',V\right>$ for
a curve $\alpha$ on $TM$
...
\item
$\left<R(X,Y)Z,W\right>=\left<R(Z,W)X,
Y\right>$,
and
...
{\left<u,u\right>\left<v,v\right>-\left<u,
v\right>^2}
\end{equation}
...
c(\left<X,Z\right>\left<Y,W\right>-\left<X,
W\right>\left<Z,Y\right>)
\end{equation}
...
$$
\left<V,W\right>_Z=\frac{\text{Re}(V,W)}{(Z,
Z)}
$$
onde
$$
...
$\left<\cdot,\cdot\right>$ restrita a
$S^{2n+1}\subset
...
$X$, $Y$, $\cos
\varphi=\left<\overline{X},i\overline{Y}
\right>$,
e $\overline{X}$,
...
=K(\left<X,J\right>\left<\gamma',
\gamma'\right>
-\left<X,\gamma'\right>\left<J,
\gamma'\right>)
$$
...
\|J(t)\|^2=\|w\|^2t^2-\frac{1}{3}\left<R(w,v)
v,w\right>t^4+O(t^4).
\end{equation}
\end{proposition}
...
que $\left<J,\gamma'\right>''=0$, pelo que
$\left<J,\gamma'\right>=a+bt$
...
\left<\xi,\alpha(X,Y)\right>=-\left<A_\xi
X,Y\right>
\end{equation}
...
\left<\tilde{R}(X,Y)Z,W\right>=\left<R(X,Y)Z,
W\right>
-\left<\alpha(X,Z),\alpha(Y,W)\right>+
\left<\alpha(X,W),\alpha(Y,Z)\right>
\end{equation}
...
K(X,Y)=\tilde{K}(X,Y)+\left<\alpha(X,Y),
\alpha(Y,Y)\right>-\|\alpha(X,Y)\|^2
\end{equation}
...
proportionally
factor is exactly $\left<A_N X,Y\right>$.
Using this and evaluating
 at an orthonormal pair $X,Y$ we obtain
\begin{equation}
\label{equation-gauss-secti-curva-shape}
K(X,Y)=\tilde{K}(X,Y)+\left<AX,
X\right>\left<AY,Y\right>-\left<AX,Y\right>^2
\end{equation}
...
$\left<Ae_i,e_j\right>=A^k_i\left<e_k,
e_j\right>=A^j_i$.)
...
$f(x):=\left<x,v\right>$ for a fixed
...
$\left<A_{N_k}E_i,E_j\right>$ since these are
...
\ref{equation-second-fundamental-form},
$$
\left<A_{n_2}e_i,e_j\right>=\left<n_k,
\alpha(e_i,e_j)\right>
...
so that
$$
\left<n_2,-\cos\theta\partial_\theta-
\sin\theta\partial_\theta\right>
=\frac{1}{\sqrt{2}}
$$
...
manifold and
$(E^p,\widetilde{\left<\cdot,\cdot\right>},
\tilde{\nabla})$
be a
...
such sphere
by $\frac{d}{dt}\left<X,\gamma'\right>=0$
and $\left<X(q),\gamma'(q)\right>=0$.
...
$$
\frac{|\left<J(s),e_i(s)\right>-\left<J(0),
e_i(0)\right>
-s\left<J'(0),e_i(0)\right>|}{s}<\varepsilon/
2
$$
and
$$
|\left<J'(0),e_i(0)\right>-\left<J'(s),e_i(s)
\right>|<\varepsilon/2
$$
...
M$ is
$$
E(s)=\int_0^a\left<f_t(s,t),f_t(s,t)\right>\,dt.
$$
\end{definition}
...
\frac{1}{2}E'(0)=-\int_0^a\left<V,
\gamma''\right>+\left<V,\gamma'\right>|_{0}^a
+\sum_{i=1}^{k-1}\left<V,\gamma'(t_{i}^-)-
\gamma'(t_i^+)\right>
\end{equation}
\end{theorem}
...
\begin{align*}
E'(s)&=
2\int_0^a\left<f_{ts},f_t\right>
=2\int_0^a\left<f_s,f_{tt}\right>-\left<f_s,
f_t\right>|_{0}^a
\end{align*}
using that
$$
\left<f_s,f_t\right>_t=\left<f_{st},
f_t\right>+\left<f_s,f_{tt}\right>
$$
If $\gamma$ is not smooth, the integral of
$\left<f_s,f_t\right>_t$ must be
...
gives
$$
0=\left<V,\gamma'\right>|_0^a
=\left<v,\gamma'(a)\right>.
$$
...
E''(0)=\int_0^a |V'|^2-\int_0^a \left<R_{\gamma'}V,V\right>+
\left<\gamma'(t),\nabla_{\partial_s}V(t)
\right>|_{0}^a+
\sum_{i}\left<V(t_i),V'(t^-_i)-V'(t^+_i)
\right>
\end{equation}
\end{theorem}
...
\begin{align*}
E''(s)&
=2\left(\int_0^a\left<f_s,f_{tt}\right>-
\left<f_s,f_t\right>|_{0}^a\right)_s
&=\int_0^a\left<f_{ss},f_{tt}\right>+
\int_0^a\left<f_s,f_{t
ts}\right>
-\left<f_{ss},f_t\right>|_{0}^a-\left<f_s,
f_{ts}\right>|_{0}^a
\end{align*}
Now substitute
$$
...
$$
\left<f_s,f_{
ts}\right>|_{0}^a=\int_0^a\left<f_s,f_{
ts}\right>_t
=\int_0^a\left<f_{st},f_{ts}\right>+
\int_0^a\left<f_s,f_{tst}\right>
$$
...
I_a(V,W):=\int_0^a
\left<V',W'\right>-\int_0^a\left<R_{\gamma'}V,
W\right>
\end{equation}
...
\begin{aligned}
I_a(V,W)&=\int_0^a\left<V,W'\right>'-\int_0^a
\left<V,W^{\prime\prime}\right>-\int_0^a
\left<R_{\gamma'}V,W\right>\\
&=\left<V,W'\right>|_{0}^a-\int_0^a
\left<V,W^{\prime\prime}\right>-\int_0^a\left<R_{\gamma'}
W,V\right>\\
&=\left<V,W'\right>|_{0}^a-\int_0^a
\left<V,W^{\prime\prime}+R_{\gamma'}W\right>
\end{aligned}
\end{equation}
\end{remark}
...
obtain
\begin{align*}
\frac{1}{2}E''(0)&=-\int_0^\ell\left<J,J''-
R_{\gamma'}J\right>
...
$$
E''(0)=\int |\dot V|^2-\int\left<R_ {\dot
\gamma}V,V\right>+\left<\dot\gamma,
...
$\left<J,\gamma'\right>=0$ and let $V$ be a
piecewise differentiable vector
field along $\gamma$ with
$\left<V,\gamma'\right>=0$. Suppose
$J(0)=0=V(0)$ and
...
and
$\left<J,\gamma'\right>=\left<\tilde{J},
\tilde{\gamma}'\right>$.
Also,
$$
J''+K J=0,\qquad
...
$$
f'=2\left<J',J\right>,\qquad
J''=2(\left<J',J'\right>+\left<J'',J\right>).
$$
...
\left<V(\ell),A_{\gamma'(\ell)}^{(2)}V(\ell)
\right>
...
$$
When computing
$V\left<\gamma',\gamma'\right>$ we obtain
$\left<\nabla^{\perp,i}_V\gamma',
\gamma'\right>=0$
...
have
$\left<\nabla_{\partial_s}V,\gamma'\right>$.
Notice that
$$
\left<f_s,f_t\right>_s=\left<f_{ss},
f_t\right>+\left<f_s,f_{st}\right>
$$
...
\[E''(0)=-\int_0^a \left<R_{\dot
\gamma}V,V\right>=-\int_0^a K<0\] já que
...
implica que existe um vetor
 $V \in T_q M$ tal que
$\left<\gamma'(\ell),V\right>< 0$ e
$\left<\sigma'(\ell),V\right>< 0$.
...
$\left<\gamma'(\ell),V\right><0$ é um
semiespaço, e o mesmo acontece com
 os vetores satisfazendo
 $\left<\sigma'(\ell),V\right><0$. Esses dois
 semiespaços
