...
\hfill
\href{http://github.com/danimalabares/vertex%
-algebras}
...
algebras}
\label{section-some-infin-dimen-lie-algeb}
Let $\mathfrak{g}$ be a finite-dimensional
...
\end{equation}
Case \ref{equation-Jacobi-algebra-d-case-1}
gives
\begin{align*}
&
...
\end{align*}
Case \ref{equation-Jacobi-algebra-d-case-2}
gives
\begin{align*}
...
\begin{exercise}
\label{exercise-invar-bilin-form-simpl}
...
\begin{exercise}
\label{exercise-affin-lie-algeb-lie-algeb}
...
Exercise
\ref{exercise-invar-bilin-form-simpl}.
\end{proof}

We also have
\begin{definition}
\label{definition-exten-affin-lie-algeb}
...
Exercise \ref{exercise-Lie-algebra-d} is
...
computation \ref{equation-Jacobi-derivation}.
In the case
...
\begin{proposition}
\label{proposition-reali-free-lie-algeb}
...
\end{align*}
such that
$\left<\alpha_i^\vee,\alpha_j\right>=a_{ij}$,
$1\leq i,j\leq n$.
\end{definition}
...
$\left<\delta,d\right>=1=\left<\Lambda_0,
K\right>$,
...
So we obtain
\begin{align*}
\left<\alpha_0^\vee,\alpha_1\right>&=\left<K-
H,\alpha_1\right>=2\\
\left<\alpha_1^\vee,\alpha_0\right>&=\left<H,
\delta-\alpha_1\right>=-2\\
\left<\alpha_0^\vee,\alpha_0\right>&=\left<K-
H,\delta-\alpha_1\right>=+2
\end{align*}
...
$[H,e_i]=\left<\alpha_i,H\right>e_i
...
defined by
$$
r_i(\lambda)=\lambda-\left<\lambda,
\alpha_i^\vee\right>\alpha_i.
$$
...
$\left<r_i|i=1,\ldots,\ell\right>\subset
\text{GL}(\mathfrak{h}^*)$.
...
k\Lambda_0+f\delta
-\left<\alpha_0^\vee,m\alpha_1+k\Lambda_0+
f\delta\right>\alpha_0\\
...
\begin{proposition}
\label{proposition-weyl-group-affin-kac}
For the affine Kac-Moody algebra
$\hat{\mathfrak{g}}$
 with
 $\hat{W}=\left<r_0,r_1,\ldots,r_\ell\right>$
 its Weyl group
(and $W=\left<r_1,\ldots,r_\ell\right>\subset
...
\begin{remark}
\label{remark-weyl-group-large-other-kac}
...
\end{equation}
The product on Eq.
\ref{equation-character-of-Verma-module} is
called
{\it Weyl denominator}.
\begin{exercise}
...
$\alpha=2\omega$.)
$L(3\omega)=\mathbb{C}\left<e^{3\omega},e^{-
\omega},e^{-3\omega}\right>$.
...
\left<\lambda,
\alpha_i^\vee\right>\in\mathbb{Z}_{\geq
0,1},
i=1,\ldots,\ell\}.
$$
\end{definition}
...
$\left<\Lambda,\alpha_i^\vee\right>
\in \mathbb{Z}_{\geq 0}$. Applying for all
$i$, we find
$\Lambda \in P_+$ is necessary.

\begin{proposition}
\label{proposition-integ-iff-domin-integ}
...
\qquad
\left<\delta,\alpha_i^\vee\right>=0,\quad
i=0,1
$$
\end{example}

\begin{remark}
\label{remark-verma-affin-kac-moody-algeb}
...
$$
P_+^k=\{\lambda \in P_+|
\left<\lambda,\theta\right>\leq k\}\subset
...
and
$$
\left<\lambda-p\alpha_i,h_i\right>=-
\left<\lambda+q\alpha_i,h_i\right>.
$$
\medskip\noindent
...
chosen so that
$\left<\rho,\alpha_i^\vee\right>=1$, and it's
called
the {\it Weyl vector}.
...
For $w \in
W=\left<r_i|i=1,\ldots,\ell\right>$,
...
\end{align*}
$$
r_i=\rho-\left<\alpha_i^\vee,
\rho\right>\alpha_i=\rho-\alpha_i
$$
Hence
$$
\sum_{\lambda \leq
...
\label{section-chara-integ-highe-weigh}
...
semidirect} product!)
$$
\widehat{W}=\left<r_0,\ldots,r_\ell\right>
\cong W \times T
$$
...
\ref{equation-weak-modular-form} and such
that
...
Then $\Gamma=\left<S,T^2\right>\subset
\text{SL}_2(\mathbb{Z})$.
...
\begin{remark}
\label{remark-dimen-numbe-parti}
...
\begin{definition}
\label{definition-creat-annih-opera}
For a quantum field
...
\medskip\noindent
\begin{definition}
\label{definition-mutua-local-quant-field}
...
\begin{proof}
First we prove \ref{equation-3} using
\ref{equation-4}.
\end{proof}

\begin{exercise}
\label{exercise-deduce-1-and-2-from-3}
Deduce \ref{equation-1} and  \ref{equation-2}
from \ref{equation-3}.
\end{exercise}
...
\begin{proposition}
\label{proposition-nth-produ-quant-field}
...
algebra}
\label{section-secon-defin-verte-algeb}
...
\label{item-verte-algeb-secon-defin-1}
\begin{align*}
...
\item
\label{item-verte-algeb-secon-defin-2}
$$
s(z(z)_{(n)}b(z)=a_{(n)}s(b(z)).
$$
\item Let $\mathcal{G} \subset
\tilde{\mathcal{F}}$ be
\label{item-verte-algeb-secon-defin-3}
...
\ref{item-verte-algeb-secon-defin-3}
...
\item Equation
\ref{equation-Borcherds-identity} holds
...
\begin{example}
\label{example-heise-verte-algeb-defin}
$V=H=\mathbb{C}[x_1,x_2,\ldots]$,
$\mathcal{F}=\{h(z)\}$ where
...
use
Borcherd's formula
\ref{equation-Borcherds-identity}.
...
\begin{align*}
\left<\cdot,\cdot\right>: \mathfrak{a}\times
\mathfrak{a}
&\longrightarrow  \mathbb{C}\\
...
Since $a,b$ odd here,
we say $\left<a,b\right>=\left<b,a\right>$
means
$\left<\cdot,\cdot\right>$ is skew-super
symmetric.
...
$\mathbb{C}^n=\left<e_1,\ldots,e_n\right>$
...
\label{section-unive-affin-verte-algeb}

Recall the construction of the affine vertex
algebra,
Definition
\ref{definition-affine-Lie-algebra}.
...
\begin{equation}
\label{equation-quant-field-unive-verte}
a^M(z)=\sum_{n \in
...
\begin{remark}
\label{remark-heise-speci-case-smoot-const}
...
\ref{equation-quant-field-unive-verte},
...
\begin{example}
\label{example-fermi-unive-verte-algeb}
...
\begin{remark}
\label{remark-resca-killi-form-coxet-numbe}
In general
...
Heisenberg. And
$F_1=F=\left<\varphi,\varphi^*\right>$
...
\begin{proposition}
\label{proposition-f0-isomo-polyn-repre}
...
\begin{proposition}
\label{proposition-isomo-repre-all-m}
...
free Fermions}
\label{section-anoth-prese-fermi}
...
cocycle}
\label{section-tate-exten-japan-cocyc}
...
Explicitly,
$$
X=\frac{K \oplus B}{\left<(t(a),0)-(0,i(a))|a
\in A\right>}.
$$
...
\begin{example}
\label{example-trace-commu-not-vanis}
...
\mathbb{C}}{
\left<(h,-h,0)=(0,0,\text{Tr}(h))|h \in
\text{End}_f(X)\right>},
$$
and there is a canonical representation of
$fl(X)^\flat$ on the semi-infinite wedge
space
$\Lambda^{\infty/2}(X)=\left<i_{i_0}\wedge
e_{i_1}\wedge\ldots\right>$
defined by
\begin{equation}
\label{equation-rep}
\begin{aligned}
...
$$
\Lambda^{\infty/2}(X)
=\left<L_{m_0}\wedge L_{m_1}\wedge
L_{m_2}\wedge\ldots\right>.
$$
...
\begin{proposition}
\label{proposition-root-syste-latti-lie}
...
\ref{item-vmod1} and \ref{item-vmod2}.
...
\end{equation}
(see Equation
\ref{equation-quantum-field-vmod}).
...
\ref{equation-quantum-field-vmod2}
does!
...
\ref{equation-mutually-localvmode}.)
That is,
\begin{equation}
...
satisfy
Equation \ref{equation-cocycle}, that is,
...
\label{section-repre-verte-algeb}
...
\ref{equation-quantum-relation}, namely:
 $$
C(\alpha+\beta,\gamma)C(\alpha,\beta)
...
\noindent
The analogue of Dong's lemma \ref{lemma-dong}
guarantees all
...
\begin{theorem}
\label{theorem-vertex-algebra-continuous}
...
\begin{proposition}
\label{proposition-smooth-f-module}
$(M,Y^M)$ is an
...
the conditions
(\ref{item-separating}) and (\ref{item-l}).
Indeed, (\ref{item-separating}) is given by
PBW theorem
since every monomial has a highest degree
term.
As for (\ref{item-l}), e.g., for $u=at^{-5}$,
$p=10$, if I take
$\ell=15$, then $u_\ell \ni bt^{15}$,
$[bt^{15},at^{-5}]=[b,a]t^{10}$.
...
their modules}
\label{section-characters-of-vertex-algebras}
...
\begin{example}
\label{example-heisenberg-character}
...
\begin{example}
\label{example-rank-n-heisenberg}
...
\begin{example}
\label{example-latti-verte-algeb-chara}
...
\begin{remark}
\label{remark-theta-function-converges}
...
\medskip\noindent
Many lectures ago
(see Section \ref{section-theta-functions}),
...
\begin{proposition}
\label{proposition-theta-funct-self-dual}
...
\begin{example}
\label{example-a2-theta-function}
...
function
(Theorem
\ref{theorem-dedekind-eta-function}).
...
\begin{definition}
\label{definition-chara-confo-verte-algeb}
...
\begin{lemma}
\label{lemma-character-v-modules}
...
\begin{proposition}
\label{proposition-latti-sl2-repre}
...
\ref{definition-modular-form}) that

\begin{definition}
\label{definition-modular-form2}
...
\begin{remark}
\label{remark-firs-eisestein-series}
...
\begin{definition}
\label{definition-space-holom-modul-forms}
...
\begin{proposition}
\label{proposition-dimension-mhol2k}
...
\label{section-a-non-modular-form}
...
\begin{equation}
\label{equation-e14}\begin{aligned}
...
Equation \ref{equation-e14}.
I.e.,
\begin{equation}
\label{equation-coefficients}
...
$W=\left<r_\alpha|\alpha \in \Delta\right>$,
or rather, $W \subset O(\mathbb{R}^n)$.
The following lemma claims that this form is
unique.

\begin{lemma}
\label{lemma-uniqueness-of-bilinear-form}
If
$\left<\cdot,\cdot\right>:\mathbb{R}
^n\times\mathbb{R}^n\to\mathbb{R}$
invariant under $W$, then
$\left<\cdot,\cdot\right>=\alpha(\cdot,\cdot)
$
...
\ref{equation-coefficients}.

\begin{example}
\label{example-coxeter-numbers}
...
\begin{example}
\label{example-simple-root-systems}
