...
subring
$A[b]=A\cdot \left<1,b,b^2,\ldots\right>$
...
\left<1,b,b^2,\ldots\right>$ is obviously
finitely
...
\begin{proposition}
\label{proposition-dvr-properties}
A discrete valuation ring is local,
Noetherian, integrally closed, all its ideals
are principal, and its Krull dimension is 1.
\end{proposition}

\begin{proof}
Step 1. Clearly, all $x\in A$ with
$\nu(x)=0$ are invertible. Therefore, the
ideal
$$
\mathfrak p:=\{x\in A\mid \nu(x)>0\}
$$
is maximal. Since all elements not in
$\mathfrak p$ are invertible, $A$ is local.

Step 2. Let $p\in A$ be an element such that
$\nu(p)=1$. Then for any $x\in A$ with
$k=\nu(x)$, the element
$$
x_1:=xp^{-k}
$$
belongs to $A$ and is invertible. Then
$x=p^kx_1$. We obtain that any ideal which
contains $p^k$ contains all elements
$x\in A$ with $\nu(x)>k$.

Step 3. Let $I\subset A$ be an ideal, and
$k:=\min_{x\in I}\nu(x)$. By Step 2,
$I=(p^k)$, hence $I$ is principal. The only
chain of ideals in $A$ is
$$
(p^{k_1})\supset (p^{k_2})\supset (p^{k_3})
\supset \cdots
$$
with $k_1>k_2>k_3>\cdots$, and it terminates
because all $k_i$ are positive integers.
Therefore, $A$ is Noetherian. It is
integrally closed because it is factorial.
\end{proof}

\medskip\noindent
The following proposition is AI-generated;
I needed it for some unrelated exercise:

\begin{proposition}
\label{proposition-one-dimensional-dvr}
Let $A$ be a Noetherian local domain of
Krull dimension $1$. Then the following are
equivalent:
\begin{enumerate}
...
\begin{proof}
\begin{proof}
A discrete valuation ring is integrally
closed by Proposition
\ref{proposition-dvr-properties}. Also, if
$A$ is a discrete valuation ring, then its
maximal ideal is generated by one element,
say $\mathfrak m=(t)$. Therefore
$$
\dim_{A/\mathfrak m}\mathfrak m/\mathfrak m^2
=1
=\dim A,
$$
so $A$ is regular local.

Conversely, if $A$ is regular local of
dimension $1$, then its maximal ideal is
generated by one element, say
$\mathfrak m=(t)$. By the Krull intersection
theorem, every nonzero $x\in A$ can be
written uniquely as
$$
x=t^n u
$$
with $u$ a unit. Hence $A$ is the valuation
ring of the valuation
$$
\nu(t^n u)=n,
$$
so $A$ is a discrete valuation ring.

Finally, the implication from integrally
closed to discrete valuation ring is the
standard one-dimensional normal local
criterion: a Noetherian local domain of
dimension $1$ is integrally closed if and
only if it is a discrete valuation ring.
\end{proof}
\end{proof}
