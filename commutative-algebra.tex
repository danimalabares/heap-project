...
\label{section-category-affine-varieties}
...
\medskip\noindent {\bf Attempt 1.} 
\begin{quote}
Let $I$ be an ideal of $M$ and $i\in I$.
Consider the class of $i$ with respect to
$N_1$, $[i]_{N_1}$. Since $M/N_1$ is
Noetherian, $I/N_1$ is finitely generated.
Thus
$$
[i]_{N_1}=\sum_j a_j[f_j]
$$
for some $f_j\notin N_1$ (otherwise they
would be the trivial class). This says that
there is $n_1\in N_1$ such that
$$
i-\sum_j a_jf_j=n_1.
$$
Now observe that $n_1$ is also in $N_2$
(assuming further, with great cheating, that
$M=N_1\oplus N_2$): for this we can just
suppose that $i$ is in $N_2$, for otherwise
we swap the roles of $N_1$ and $N_2$ in the
whole argument; and also clearly
$\sum_j a_j f_j$ is in $N_2$ since all the
$f_j$ cannot be in $N_1$, as I explained
above. In conclusion, $n_1$ is also in
$N_2$, which makes it $0$. Then
$$
i=\sum_j a_jf_j.
$$
Any other $i'\in I$ would be either
generated this way or similarly by some
elements in $N_2$.
\end{quote}
...
=\sum_k b_kg_k+n_2.
$$
So initially I said, well OK, let's simplify
things: suppose $i$ is in $N_1$, then we get
$$
i-\sum_k b_kg_k=n_2.
$$
Now, $i$ being in $N_1$ means it is not in
$N_2$. And also $g_k$ cannot be in $N_2$
because it is a nontrivial class in $M/N_2$,
so I thought, wrong, because the sum of the
$g_k$ \emph{could} be in $N_2$, that
$i-\sum_k b_kg_k$ would be not in $N_2$,
but being equal to $n_2$, we must have that
$n_2=0$. This would conclude the argument
but again, it is wrong.

In the general case when $i$ is not in
$N_1$ nor in $N_2$ I still struggle with the
fact that the sum of elements of submodules
can leave the submodules.
\end{quote}
...
\begin{exercise}
\label{exercise-coproduct-in-rings}
...
\begin{definition}
\label{definition-torsion-module}
...
\begin{exercise}
\label{exercise-flat-modules-torsion-free}
...
\begin{exercise}
\label{exercise-union-flat-modules-flat}
Let
$
...
\begin{exercise}
\label{exercise-flat-finite-local-free}
...
$\varphi: M_1 \otimes_A M$ is injective,
i.e. that if $\varphi(m_1 \otimes m)=0$
then $m_1 \otimes m =0$.
The idea is that this tensor,
$m_1 \otimes m$, is zero when the
coefficients are any of the localizations
$A_{\mathfrak{p}}$.
So basically the claim is:
if the tensor is zero in all localizations
then it's zero.
\end{proof}
...
\label{section-kahler-differentials}
...
\begin{lemma}
\label{lemma-derivations-polynomial-ring}
...
\begin{proposition}
\label{proposition-kahler-differentials-polynomial-ring}
Let $R=k[t_1,\dots,t_n]$.
...
\label{section-zariski-cotangent-space}

\begin{definition}
\label{definition-zariski-cotangent-space}
...
\begin{proposition}
\label{proposition-zariski-cotangent-space-kahler}
...
\begin{lemma}[Corollary]
\label{lemma-zariski-cotangent-space-manifold}
...
\begin{proposition}
\label{proposition-regular-points-affine-variety}
...
$\dim X = n-\dim W$
(Theorem \ref{theorem-regular-rings}).
\end{proposition}
...
\begin{lemma}[Corollary]
\label{lemma-regular-values-affine-variety}
Let $X \subset \mathbf{C}^n$ be an affine
variety defined by a system of polynomial
equations
...
\begin{definition}
\label{definition-smooth-point-variety}
...
\label{section-conormal-exact-sequence}
...
\begin{definition}
\label{definition-relative-kahler-differentials}
Let $f:X \to Y$ be a morphism of complex
affine varieties associated with a ring
homomorphism $f^\ast:B\to A$. The module
...
\begin{remark}
\label{remark-relative-kahler-differentials}
...
\label{proposition-conormal-exact-sequence}
...
\begin{lemma}[Corollary]
\label{lemma-conormal-exact-sequence-ideal}
Let $X \hookrightarrow Y$ be a variety
defined by the ideal $I$. Then there is a
natural exact sequence
$$
I/I^2 \to \Omega^1_X \to \Omega^1_{X/Y}\to 0.
$$
\end{lemma}

\begin{proof}
Putting $Z=\operatorname{Spec}(\mathbf C)$,
and writing the conormal exact sequence,
we get
$$
...
$$
The surjective map
$I/I^2 \to \Omega^1_Y\otimes_{\mathcal O_Y}
\mathcal O_X$
was constructed in the proof of the
conormal exact sequence.

(It's not clear whether this proof
is correct; in lecture we did a direct proof
not using the conormal exact sequente.)
\end{proof}

\begin{lemma}[Corollary]
\label{lemma-cotangent-space-point}
Let $y$ be a point on $Y$.
Then $\Omega^1_{\{y\}/Y}=T_y^\ast Y$.
\end{lemma}

\section{Étale maps}
\label{section-etale-maps}
...
\begin{theorem}
\label{theorem-etale-cotangent-isomorphism}
...
\begin{lemma}[Corollary]
\label{lemma-etale-smooth-points}
...
\begin{exercise}
\label{exercise-derivations-finite-field-extension}
...
\begin{definition}
\label{definition-ramification-locus}
...
\begin{proposition}
\label{proposition-ramification-locus-not-dense}
...
\begin{lemma}[Corollary]
\label{lemma-finite-map-etale-outside-closed}
Any finite map is \'etale outside of a
Zariski closed set.
\end{lemma}

\begin{lemma}[Corollary]
\label{lemma-smooth-points-zariski-dense}
