...
Here Sernesi explains the main idea:
``The approach of Grothendieck to this
problem was to formalize the method
of Kodair and Spencer, which consists
in a formal construction
followed by a proof of convergence''.

Thus the following definitions explain
the general pipeline we use in deformation
theory: start with the more manageable 
finite-order infinitesimal deformations
and produce a ``formal deformation'',
which is an object encoding the information
of all finite lifts.
 
It remains to pass from this formal
deformation to an actual deformation, which
``is the analogous of the convergence step
in the Kodaira-Spencer theory, and is a very
difficult problem, the {\it algebraization
problem}''.
...
\item Let $(R,\mathfrak m)$ be a complete
Noetherian local $\mathbb C$-algebra with
residue field $\mathbb C$. A {\it formal
deformation} of $X$ over $R$ is a compatible
sequence
$$
{\xi_n}*{n\geq 0},
$$
where $\xi_n$ is an infinitesimal deformation
of $X$ over
$$
\Spec(R/\mathfrak m^{n+1}),
$$
and $\xi_n$ induces $\xi*{n-1}$ by base
change under the quotient map
$$
R/\mathfrak m^{n+1}\longrightarrow
R/\mathfrak m^n.
$$
Equivalently, it may be viewed as a flat
...
\section{The second-order lifting equation}
\label{section-second-order-lifting}
...
I=(f_1,\ldots,f_{16}),
$$
where the generators $f_i$ are cubic.
Choose representatives
$\phi_1,\ldots,\phi_{53}$ of a basis of
$T^1_{A,0}$ and write
$$
g(y)=\sum_{j=1}^{53}y_j\phi_j.
$$
Thus the corresponding first-order
perturbations of the generators are
$$
f_i+t g_i(y).
$$
...
\sum_i s_{ri}f_i=0
$$
be one of the original first syzygies.
Since $g(y)$ is a first-order deformation,
we can also lift the coefficients of this
syzygy. Therefore we choose
$\alpha_{ri}(y)$, linear in $y$, such that
$$
\sum_i s_{ri}g_i(y)
+
\sum_i\alpha_{ri}(y)f_i
=0.
$$
Equivalently,
$$
\sum_i
\bigl(s_{ri}+t\alpha_{ri}(y)\bigr)
\bigl(f_i+t g_i(y)\bigr)
=0
\pmod{t^2}.
$$

We now try to extend the deformation to
second order:
$$
f_i+t g_i(y)+t^2h_i.
$$
The syzygies must also be extended to the
form
$$
s_{ri}+t\alpha_{ri}(y)+t^2\beta_{ri}.
$$
The coefficient of $t^2$ in the resulting
syzygy equation is
$$
\sum_i s_{ri}h_i
+
\sum_i\alpha_{ri}(y)g_i(y)
+
\sum_i\beta_{ri}f_i.
$$
After passing to $A=S/I$, the last term
vanishes. Hence the second-order equation is
$$
\sum_i s_{ri}h_i
+
\sum_i\alpha_{ri}(y)g_i(y)
...
Collecting these equations for all the
original first syzygies defines the linear
map
$$
B(h)=
\left(
\sum_i s_{ri}h_i\pmod I
\right)_r.
$$
Thus $B$ measures the syzygy errors that can
be removed by choosing the second-order
correction $h$.

Both $\alpha(y)$ and $g(y)$ are linear in
$y$. Their product is therefore quadratic.
Let
$$
q(y)=(y_i y_j)_{1\leq i\leq j\leq53}.
$$
The vector $q(y)$ has
$$
\binom{54}{2}=1431
$$
entries. After choosing bases, there is a
matrix $Q_0$ such that
$$
Q_0q(y)=
\left(
\sum_i\alpha_{ri}(y)g_i(y)\pmod I
\right)_r.
$$
The second-order lifting equation is therefore
$$
Bh+Q_0q(y)=0.
$$

Consequently, the first-order deformation
$g(y)$ lifts to second order precisely when
$$
Q_0q(y)\in\operatorname{im}B.
$$

The target of $B$ is a raw space of
coefficients of syzygy errors. It contains
many redundant directions and is not itself
the obstruction space $T^2_{A,0}$.
...
(Yes, for some reason there's only
one deformation paramter.)
