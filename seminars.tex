...
is
$$
\Sigma_\Pi:=\{p\in\mathbb{S}^n:\left<p,
x\right>=0\forall
x\in\Pi\}
$$
...
Now define
$$
\Sigma_V=\{p\in\mathbb{S}^n:\left<p,v\right>=
0\}=V^{-1}(0)
$$
where $V(x):=\left<x,v\right>$ for all
...
\item $\left<\cdot,\cdot\right>:TM \otimes
T^*M \to \mathbb{R}_M$
is the dual pairing.
...
a quasi isomorphism.
\item
$\left<\cdot,\cdot\right>:\mathbb{V}\otimes
\mathbb{V}^{n*}\to
...
$$
\left<\cdot,\cdot\right>:T\mathcal{G} \otimes
T^{n*}\mathcal{G}
...
$N=\left<a\right>\times \left<\lambda\right>$
...
$\left<\cdot,\cdot\right>_R: R \otimes R \to
\mathbb{R}$.
Take $\mathfrak{g}=R \oplus R^*$,
and pick $H \in \Lambda^{3}R^*$.
Put $H(a,b,c)=\left<[a,b],c\right>$, and
define
$$
...
\left<\varphi\right>$.
$\phi(x,y,z)=\left<x\cdot y\cdot z\cdot
\zeta,\zeta\right>$.
...
$$
This determines $d
d^c\omega=\alpha\left<F_\theta \wedge
F_\theta\right>$.
...
\begin{theorem}
\label{theorem-theor-lagra-blow-up-well}
