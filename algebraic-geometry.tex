...
\begin{proof}
\begin{enumerate}
\item (For the record, I'll
keep my original attempt of copying
the affine case to the projective case
- this doesn't work.)
Suppose $x \in
\text{Frac}(\mathcal{O}_P)$
is integral, i.e. there is a monic
polynomial $P=\sum_i \frac{a_i}{b_i}t^i
\in \mathcal{O}_P[t]$ vanishing at $x$.
Note that $x \in \text{Frac}S(Y)$.
Multiplying $P$ with the least common
multiple of the $b_i$ we obtain a polynomial
with coefficients in $S(Y)$ vanishing
at $x$. As in the affine case,
to obtain a monic polynomial and use
integrality of $S(Y)$ we must consider
instead of $x$, its product with
$lcm(b_i)^{\text{deg}P}$.
Since $S(Y)$ embeds in $\mathcal{O}_P$
(that's the mistake! $S(Y)$ cannot
be seen as regular functions for projective
varieties)
we conclude
that $lcm(b_i)^{\text{deg}P}x 
\in \mathcal{O}_P$
which means that $x \in \mathcal{O}_P$.

The correct solution requires us
to take an affine chart at some point.
Then all we have to do is check
that the affine variety corresponding to $Y$
in this chart is normal. The ideal
of such a variety is obtained
by adding the ideal of $Y$ with $(x_i-1)$
for some $i$ --- this corresponds to
intersecting with the plane $x_i=1$.
The resulting algebra is not immediately 
integral! --- we must use proposition
\ref{?} to view such an algebra as a 
0-localization…
we can readily use integrality of $S(Y)$
by noting that the monic polynomial
of any integral element of the restricted
algebra is also a monic polynomial with
coefficients in $S(Y)$, which means that
the denominator in the fraction is a unit
of $S(Y)$ and thus a unit of the restricted
algebra as well.

\item To show that the twisted quartic
has all stalks which are integrally closed
it's enough to show it's smooth,
which in turn makes stalks be UFDs.
It is clearly smooth since all its
coordinate functions are smooth.
To show it's not projectively normal
we must have a look at its coordinate
algebra… but unfortunately this is given
by a determinantal ideal… the minors
of the catalecticant matrix…

\item The isomorphism is clear:
in fact the parametrization is a
biholomorphism since it is surjective
on its image and locally invertible.
We can confirm that $\mathbb{P}^1$ is
projectively normal since we can embed it
in $\mathbb{P}^3$ with ideal $(x_0,x_1,x_2)$,
giving coordinate algebra $k[t]$.
\end{enumerate}
\end{proof}
