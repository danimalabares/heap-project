...
Let $k$ be a field. The \emph{ring of dual
numbers over $k$} is
$$
D=k[t]/(t^2).
$$
A \emph{first-order deformation} of a
structure over $k$ is a flat extension of
that structure over $\operatorname{Spec}D$
whose special fiber is the original
structure.
\end{definition}

\begin{lemma}
\label{lemma-flatness-tor}
Let $A$ be a noetherian ring and let $M$ be
an $A$-module. Then $M$ is flat over $A$ if
and only if
$$
\operatorname{Tor}^A_1(M,A/\mathfrak{p})=0
$$
...
\begin{lemma}
\label{lemma-square-zero-flatness}
Let $A'\to A$ be a surjective homomorphism
of noetherian rings with square-zero kernel
$J$. Let $M'$ be an $A'$-module and set
$$
M=M'\otimes_{A'}A.
$$
Then $M'$ is flat over $A'$ if and only if
$M$ is flat over $A$ and the natural map
$$
...
\label{lemma-dual-numbers-flatness}
Let $M'$ be a $D$-module and set
$M_0=M'/tM'$. Then $M'$ is flat over $D$
...
$Y\subset X$ be a closed subscheme. Set
$$
X'=X\times_k\operatorname{Spec}D.
$$
An \emph{embedded first-order deformation
of $Y$ in $X$} is a closed subscheme
$Y'\subset X'$ that is flat over $D$ and
satisfies
$$
Y'\times_D k=Y.
$$
...
\begin{theorem}
\label{theorem-affine-embedded-first-order}
Let $X=\operatorname{Spec}B$, and suppose
that $Y$ is defined by an ideal $I\subset B$.
The embedded first-order deformations of
$Y$ in $X$ are naturally classified by
$$
\operatorname{Hom}_B(I,B/I).
$$
The zero element corresponds to the trivial
...
Let $Y\subset X$ have ideal sheaf
$\mathcal{I}$. The \emph{normal sheaf of
$Y$ in $X$} is
$$
\mathcal{N}_{Y/X}
=
\mathcal{H}om_{\mathcal{O}_Y}
(\mathcal{I}/\mathcal{I}^2,
\mathcal{O}_Y).
$$
If $X$ is nonsingular and $Y$ is a local
complete intersection, this locally free
sheaf is the \emph{normal bundle of $Y$ in
$X$}.
\end{definition}
...
Let $X$ be a scheme over $k$, and let
$Y\subset X$ be a closed subscheme. The
embedded first-order deformations of $Y$ in
$X$ are naturally classified by
$$
H^0(Y,\mathcal{N}_{Y/X}).
$$
The zero element corresponds to the trivial
deformation.
\end{theorem}
...
Let $Y\subset\mathbb{P}^n_k$ be a closed
subscheme, and let $y$ be the corresponding
point of the Hilbert scheme. Then
$$
T_y\operatorname{Hilb}(\mathbb{P}^n_k)
\cong
H^0(Y,\mathcal{N}_{Y/\mathbb{P}^n_k}).
$$
\end{lemma}

\begin{definition}
\label{definition-line-bundle-first-order}
Let $L$ be an invertible sheaf on $X$. A
\emph{first-order deformation of $L$} is an
invertible sheaf $L'$ on
$$
X'=X\times_k\operatorname{Spec}D
$$
whose restriction to $X$ is isomorphic to
$L$.
\end{definition}

\begin{theorem}
\label{theorem-line-bundle-first-order}
The isomorphism classes of first-order
deformations of an invertible sheaf $L$ on
$X$ are naturally classified by
$$
H^1(X,\mathcal{O}_X).
$$
...
Let $F$ be a coherent sheaf on $X$. A
\emph{first-order deformation of $F$} is a
coherent sheaf $F'$ on
$$
X'=X\times_k\operatorname{Spec}D
$$
that is flat over $D$, together with a map
$F'\to F$ inducing an isomorphism
$$
F'\otimes_D k\xrightarrow{\sim}F.
$$
Two deformations are \emph{equivalent} if
there is an isomorphism between them that is
compatible with the maps to $F$.
\end{definition}

\begin{theorem}
\label{theorem-coherent-first-order}
The equivalence classes of first-order
deformations of a coherent sheaf $F$ on
$X$ are naturally classified by
$$
\operatorname{Ext}^1_X(F,F).
$$
The zero element corresponds to the trivial
deformation.
\end{theorem}
...
\begin{theorem}
\label{theorem-unmarked-coherent-first-order}
If the identification of the special fiber
with $F$ is not specified, the isomorphism
classes of first-order deformations are the
orbits of $\operatorname{Ext}^1_X(F,F)$
under the action of $\operatorname{Aut}F$.
\end{theorem}

\begin{lemma}[Corollary]
\label{lemma-vector-bundle-first-order}
Let $E$ be a vector bundle on $X$. Its
first-order deformations are naturally
classified by
$$
H^1(X,\mathcal{E}nd E).
$$
The zero element corresponds to the trivial
deformation.
\end{lemma}
