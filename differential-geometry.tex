...
$$
\frac{1}{2}E'(s)
=\left<f_s,f_t\right>|_0^a
-\int_0^a\left<f_s,f_{tt}\right>.
$$
...
\frac{1}{2}E''(s)&
=\left(
\left<f_s,f_t\right>|_{0}^a
-\int_0^a\left<f_s,f_{tt}\right>\right)_s\\
&=(\left<f_s,f_t\right>|_{0}^a)_s
-\int_0^a\left<f_{ss},f_{tt}\right>
-\int_0^a\left<f_s,f_{tts}\right>
\end{align*}
Now substitute
using the Symmetry Lemma for Curvature
\ref{lemma-symmetry-lemma-curvature}:
$$
R(\gamma',V)\gamma'
=f_{t
ts}-f_{tst}
...
(\left<f_s,f_t\right>|_{0}^a)_s
-\int_0^a \left<f_{ss},f_{tt}\right>
-\int_0^a\left<f_s,R(s,t)f_t\right>
-\int_0^a\left<f_s,f_{t s t}\right>
$$

Now use that the connection is metric
to make the term $\left<f_s,f_{tst}\right>$ 
appear by differentiating 
$\left<f_s,f_{t s}\right>$
with respect to $t$:
$$
\left<f_s,f_{ts}\right>_t
=\left<f_{st},f_{ts}\right>
+ \left<f_s,f_{t s t}\right>.
$$
We obtain
$$
\frac{1}{2}E''(s)=
...
-\int_0^a \left<f_{ss},f_{tt}\right>
-\int_0^a\left<f_s,R(s,t)f_t\right>
+\int_0^a\left<f_{s t},f_{t s}\right>
-\left<f_s,f_{t s}\right>|_0^a.
$$
Now note that $\left<f_s,f_{t s}\right>|_0^a$
cancels out with the very first term,
which is
$$
(\left<f_s,f_t\right>|_0^a)_s
=\left<f_{s s},f_t\right>|_0^a
+\left<f_s,f_{t s}\right>|_0^a,
$$
giving
$$
\frac{1}{2}E''(s)=
\left<f_{ss},f_t\right>|_{0}^a
-\int_0^a \left<f_{ss},f_{tt}\right>
-\int_0^a\left<f_s,R(s,t)f_t\right>
+\int_0^a\left<f_{s t},f_{t s}\right>.
$$
Since $\gamma$ is a geodesic the
$\left<f_{ss},f_{tt}\right>$ vanishes
...
\begin{remark}
\label{remark-second-variation-non-geodesic}
If $\gamma$ is not a geodesic we may
use again that the connection
is metric to deal with the
$\left<f_{ss},f_{tt}\right>$ term:
$$
\left<f_{ss},f_t\right>_t
=\left<f_{ss t},f_{t}\right>
+\left<f_{ss},f_{tt}\right>
$$
gives
$$
\begin{aligned}
\frac{1}{2}E''(s)&=
\left<f_{ss},f_t\right>|_{0}^a
-\left<f_{ss},f_t\right>|_0^a
+\int_0^a \left<f_{ss t},f_t\right>\\
&-\int_0^a\left<f_s,R(s,t)f_t\right>
+\int_0^a\left<f_{s t},f_{t s}\right>.
\end{aligned}
$$
