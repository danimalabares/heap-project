...
def computeTagStats():
  ...
  for tag in Tag.select():
    ...
    TagStatistic.create(tag=tag, statistic="chapters", value=len({chapters[result] for result in preliminaries[tag.tag] if result in chapters}))
    TagStatistic.create(tag=tag, statistic="sections", value=len({sections[result] for result in preliminaries[tag.tag] if result in sections and len(sections[result].split(".")) == 2}))
...
def computeBookStats():
  ...
  if os.path.isfile(gerby.configuration.PDF):
    with open(gerby.configuration.PDF, "rb") as pdf:
      if PdfReader is None:
        pages = PdfFileReader(pdf).getNumPages()
      else:
        pages = len(PdfReader(pdf).pages)
    BookStatistic.create(statistic="pages", value=pages)
  else:
