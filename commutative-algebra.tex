...
Notice that we can see $y \in Y$ as a subvariety.
In fact, it is, like $X$, a variety {\it over $Y$}.
Thus this expression for $f^{-1}(y)$
matches that of the fibered product
in Proposition \ref{proposition-fibered-product}. 
\end{remark}
...
\noindent
It looks like the aim of this section
is to prove that a commutative algebra over a field,
which is finite-dimensional as a vector space, 
is a direct sum of fields,
namely Theorem \ref{theorem-artinian-semisimple-is-direct-sum-of-fields}.
This is used to prove that
the preimage of a point by a finite map of
affine algebraic varieties
is a finite set of points,
Theorem \ref{theorem-finite-map-has-finite-preimage}.

Moreover, it also used quite a lot in the
development of Galois theory in
Section \ref{section-field-extensions}.
...
is called an {\it Artinian ring} if it is finite-dimensional
...
the direct sum of the rings equipped with natural operations)
...
Let $v \in R$ be an element of a finite-dimensional algebra $R$ over $k$.
Let $k[v]$ be the subalgebra generated by $1, v, v^2,…$.
Prove that $P(v)=0$ for a unique monic polynomial $P$ of degree
...
\noindent
The polynomial above is called the {\it minimal polynomial} of $v$.

\begin{exercise}
\label{exercise-minimal-polynomial-and-residues}
Let $v \in R$ be an element of an Artinian ring over $k$,
and $P(t)$ its minimal polynomial.
Prove that $k[v]$ is isomorphic to $k[t]/(P)$.
\end{exercise}
...
Let $R=k[t]/P$ where $P \in k[t]$ is a polynomial
decomposing as a product $P=P_1…P_n$ of coprime polynomials.
Prove that there exists an isomorphism
$R \to \bigoplus_ik[t]/P_i$.
\end{exercise}

\begin{proof}
The natural map $[F]_P \mapsto ([F]_{P_1},…,[F]_{P_n})$
is well defined.
It is surjective and injective by the Chinese reminder theorem.
If $[F]_P = [t]_P$, i.e.
...
\begin{exercise}
\label{exercise-artinian-without-idempotents-is-field}
Let $R$ be a semisimple Artinian ring with all idempotents
equal to 1 or 0. Prove it is a field.
\end{exercise}

\begin{proof}
Suppose $R$ is not a field, thus there is an element $x$ which
is not invertible. Consider the algebra $k[x]$ generated by $x$.
By a Exercise \ref{exercise-minimal-polynomial-and-residues}
we know that $k[x] \simeq k[t]/P$ where $P$ is the minimal polynomial
of $x$. 
Now decompose $P$ in its unique irreducible factorization.
Since there are no nilpotents in $R$,
there are no nilpotents in $k[x] \simeq k[t]/P$,
which implies that the irreducible factorization of $P$
is square-free, which means that the factors of $P$
are coprime and we can apply the Chinese Reminder theorem.
That is, by Exercise \ref{exercise-chinese-reminder},
we know that $k[t]/P$ is a direct sum of fields
$\bigoplus k[t]/P_i$.
There is an idempotent element here:
$(1,0,…,0)$.
\end{proof}
...
Let $f:X \to Y$ be a finite morphism of affine varieties.
...
$R:= \mathcal{O}_X \otimes_{\mathcal{O}_Y}\mathcal{O}_Y/\mathfrak{m}_y$,
which is the coordinate ring of $f^{-1}(y)$ 
by Proposition \ref{proposition-preimage-of-point}
or Lemma \ref{lemma-preimage-of-affine-subvariety}.

Further, it is finitely generated because $\mathcal{O}_X$ is finitely generated
over $\mathcal{O}_Y$
(Remark \ref{remark-finite-generated-and-tensor-product}).

Since $\mathcal{O}_Y/\mathfrak{m}_y \simeq \mathbb{C}$,
$R$ is an Artinian algebra.
By Theorem \ref{theorem-artinian-semisimple-is-direct-sum-of-fields},
...
However, our course formulation is the other way around:

\begin{definition}
\label{definition-integral-element}
Let $A \subset B$ be rings.
An element $b \in B$ is called {\it integral over $A$} 
if the subring $A[b]=A\cdot \left<1,b,b^2,…\right>$
is generated by $b$ and $A$,
is finitely generated as an $A$-module.
\end{definition}

\noindent
Note that if $A$ is finitely generated,
$A \left<1,b,b^2,…\right>$
is obviously finitely generated as a ring,
but not as a module, which is what we care about.

\begin{lemma}
\label{lemma-integral-element-iff-chain-of-submodules}
An element $x \in B$ is integral over $A \subset B$
if and only if
...
$$
terminates.
Equivalently, $x$ is the root of a monic polynomial with coefficients in $A$.
\end{lemma}

\begin{proof}
The chain statement is almost tautological
(recall that being noetherian is the same as…).
As for the second,
express $x^n$ as an element of the finitely-generated
module.
\end{proof}

\begin{lemma}
\label{lemma-sums-and-products-of-integral-elements}
In Noetherian rings, sums and products of integral
...
let $[k:k]$ be a finite extension over with $\text{char} k =0$.
then there exists an element $l \in k$ generating $k$.


\medskip\noindent
Recall that we defined a field $k$ to be algebraically closed
it all algebraic extensions of $k$ are trivial.

\begin{lemma}
\label{lemma-algebraically-closed-iff-factoring-polynomials}
A field $k$ is algebraically closed if and only if
any polynomial $P \in k[t]$ is a product of linear factors.
\end{lemma}

\begin{proof}
If $P$ does not decompose,
there is a polynomial of degree >1 diving $P$,
and the quotient $k[t]/$such polynomial is an algebraic field extension.
\end{proof}

\noindent
There is a good notion of algebraic closure of a field
obtained by adding roots and applying Zorn's lemma
to ensure this process finishes. And it is unique.

\begin{theorem}
\label{theorem-separable-extension-and-algebraic-closure}
Let $[\bar{k}:k]$ be the algebraic closure of $k$,
...
that there only finitely many field extensions.
Namely Galois theory says that field extensions
...
Let $k$ be a field,
and $A = \bigoplus_{i=1}^nk$. Then $A$ contains
...
\begin{theorem}
\label{theorem-finite-extension-char-0-generated-by-one-element}
let $[k:k]$ be a finite extension over with $\text{char} k =0$.
then there exists an element $l \in k$ generating $k$.
\end{theorem}
...
if $V_i$ are positive codimension subspaces of $k^n$,
then $\bigcup V_i \neq  k^n$.
\end{proof}
...
It is called a {\it Galois extension} if the algebra
$K \otimes_k K$ is isomorphic to a direct sum of several copies of $K$.
\end{definition}

\begin{example}
\label{example-galois-extension}
Let $K=k[t]/(P)$
be a primitive, separable extension with $\text{deg} P=n$.
Suppose that $P$ has $n$ roots in $K$.
Then $[K:k]$ is Galois.
Indeed, this is the Chinese reminder exercise once again:
$$
...
{\it Splitting field} is the minimal extension
such that $P$ factors into linear factors.

Equivalently, pick $P \in k[t]$ of degree $n$ without multiple roots,
and $k$ of characteristic 0.
Decompose $P$ into irreducible factors and construct finite extensions
$K_1\subset … \subset K_\ell$ by adding the roots of each factor.
This process ends (prove this) and the resulting field is
the {\it splitting field}.
\end{definition}

\noindent
Splitting field is Galois by Exercise \ref{exercise-galois-extension}.
We can think of the splitting field as embedded in $\bar{k}$.
There is not a unique embedding but all embeddings are isomorphic.

\begin{definition}
\label{definition-galois-group}
If $[K:k]$ is a Galois extension,
$\text{Aut}_k K$ is called {\it Galois group}.
\end{definition}

\noindent
Note that $K^*$ acts on $K \otimes_k K$ by multiplication
...
which are linear with respect to the left action of $K$.
\end{enumerate}
\end{lemma}
...
Then the left $\mu_l$ and right $\mu_r$ actions of $K^*$ on
...
Let $[K:k]$ be a Galois extension,
and $a \in K$ a $\text{Aut}_k K$-invariant element.
Then $a \in k$.
\end{lemma}

\begin{lemma}
\label{lemma-galois-and-g-invariants}
Let $[K:k]$ be a Galois extension, and $K'$ an intermediate field
(which, as we will see in the proof, will also be Galois)
Then 
$$
K' = K^{G'}
$$
where $G' \subset \text{Aut}_{K'}K$ is the group
...
Let $[K:k]$ be a Galois extension.
The subgroups $H \subset \text{Aut}_k K$ are in bijective correspondence
...
\noindent
Recall that $\Spec(R)$ is the set of all prime ideals of the ring $R$.
We introduce a topology whose closed sets
are the prime ideals contained in a given ideal.
...
\begin{proposition}
\label{proposition-dominant-iff-induced-map-on-rings-is-injective}
A morphism of varieties $f:X \to Y$
is dominant if and only if
$f^*:\mathcal{O}_Y \to \mathcal{O}_X$ is injective.
\end{proposition}

\begin{proof}
Suppose $f^*$ is not injective and
consider its kernel $\Ker f^*$.
Then the image of $f$ is in the zero set of the kernel,
$V(\Ker f^*)$.
Here we use that points of a variety are 
homomorphisms from the variety to $\mathbb{C}$,
which follows from the Nullstellensatz…
Dominantness means that the image is not contained in
the zero set of an ideal.
\end{proof}

\begin{lemma}
\label{lemma-dominant-morphism-maps-irreducible-to-irreducible}
If $f:X \to Y$ is dominant and $X$ is irreducible,
then $Y$ is irreducible.
Moreover, $f^*:\mathcal{O}_Y \to \mathcal{O}_X$
can be extended to the fields of fractions
$k(Y) \to k(X)$.
\end{lemma}
...
i.e. an embedding,
irreducibility transforms to not having zerodivisors.
...
All of a sudden we consider a divisor on the affine variety $X$
given by $f \in \mathcal{O}_X$.
Then $X\setminus D$ is also an affine variety given by the ideal
$I+\left<ft-1\right> \subset \mathbb{C}[x_1,…,x_n,t]$.
Recall that the localization of a ring by an element is
$\mathbb{C}/\left<1-ft\right>$, where $t$ is the inverse of $f$.
\end{remark}

\noindent
We also define {\it birational morphism} if the induced map
on fraction fields is an isomorphism, and further show:

\begin{proposition}
\label{proposition-birational-then-isomorphism-away-from-divisor}
Let $f:X \to Y$ be a birational morphism.
Then there exists a divisor $Z \subset Y$
such that $f:(X \setminus f^{-1}(Z)) \to Y \setminus Z$
is an isomorphism.
\end{proposition}

\medskip\noindent
Now we introduce normal varieties.
Recall that we already discussed integral closure.

\begin{definition}
\label{definition-normal-variety}
An irreducible affine algebraic variety is called {\it normal} 
if its coordinate ring is integrally closed,
that is, it coincides with its its integral closure,
which in turn is the set of all elements that are integral
over its field of fractions.
\end{definition}

\noindent
Recall from \cite[Exercise 3.17, Chapter 1]{har}
that for affine varieties this is equivalent to all
the local rings are all integrally closed.

\begin{proposition}
\label{proposition-normal-iff-birational-morphism}
An affine variety $X$ is normal if and only if
for any finite, birational morphism
$Y \to X$ is an isomorphism.
\end{proposition}

\begin{proof}
They have the same fraction field, and $\mathcal{O}_X \subset \mathcal{O}_Y$
by dominant (birational maps are dominant),
which ways that $\mathcal{O}_Y$ is finite over $\mathcal{O}_X$,
which says that elements of $\mathcal{O}_Y$ are integral.
Since $X$ is normal then the elements of $\mathcal{O}_Y$
belong to $\mathcal{O}_X$.
\end{proof}

\noindent
Now we prove: smooth implies normal!!!

\begin{proposition}
\label{proposition-ufd-is-integrally-closed}
A UFD is integrally closed.
\end{proposition}

\noindent
As a corollary, we see that taking product of an affine variety,
whose coordinate ring is an UFD,
with affine space, $X \times \mathbb{C}^n$
preserves normality
since the resulting coordinate ring is
$\mathcal{O}_X[x_1,…,x_n]=\mathcal{O}_X \otimes \mathbb{C}[x_1,…,x_n]$.
