...
\begin{definition}
\label{definition-equivalence-of-categories}
Two categories are {\it equivalent} if there exist functors
from one to the other and backwards which composed with
...
\noindent
General fibration: every map on the base has a lift that is $p$-cartesian.
(Discrete fibration: unique?)
Fibered in groupoids: all maps in the Étale space are $p$-cartesian.

\medskip\noindent
And to my surprise, these are none other than 
categories fibered in groupoids according to
Stacks Project 
\href{https://stacks.math.columbia.edu/tag/003S}{003S} section
on Categories fibered in groupoids.

\medskip\noindent
Let $p:\mathcal{E} \to \mathcal{B}$ be any functor.
The idea of fibrations is to lift maps from $\mathcal{B}$ to $\mathcal{E}$.
We will fix the target; the condition is that for maps on $\mathcal{B}$
with target $p(y)$ there exist 
(unique? that's the condition for discrete fibrations)
maps on $\mathcal{E}$ corresponding via $p$.
Why fix the target? Because it's like a base-change.
But not only that: we need that the map that exists
is good; it has to be $p$-Cartesian: 

\begin{definition}
\label{definition-p-cartesian}
Let $p:\mathcal{E} \to \mathcal{B}$. A morphism $f$ in  $\mathcal{E}$ 
is {\it $p$-cartesian} if for all $g$ and for all $h'$ 
...
\begin{definition}
\label{definition-fibration}
$p:\mathcal{E} \to \mathcal{B}$ is a {\it fibration} if
for all $Y$ and all $f':X' \to p(Y)$ there exists $X$ and
$f:X \to Y$ such that
\begin{enumerate}
\item $p(f)=f'$, and
\label{item-fibration1}

 \item $f$ is $p$-Cartesian.
\label{item-fibration2}
\end{enumerate}
\end{definition}

\noindent
Recall Definition \ref{definition-discrete-fibration} of discrete fibration:
$p:\mathcal{E} \to \mathcal{B}$ is a discrete fibration if for all 
$Y \in \mathcal{E}$ 
and for all $f':X' \to p(Y)$ there exist 
{\bf unique} ($X$ and  $f:X \to Y$) such that $p(f)=f'$.

\begin{lemma}
\label{lemma-discrete-fibration-is-fibration}
If $p:\mathcal{E} \to \mathcal{B}$ 
is a discrete fibration then it is a fibration,
i.e.  $f$ is also $p$-Cartesian.
\end{lemma}

\begin{proof}
Uniqueness ensures source and target coincide in all lifts.
\end{proof}

\begin{lemma}
\label{lemma-composition-p-cartesian}
Composition of $p$-Cartesian morphisms is $p$-Cartesian.
\end{lemma}

\begin{proof}
Write the diagrams and don't forget to prove uniqueness.
\end{proof}

\begin{lemma}
\label{lemma-2-for-3-p-cartesian}
...
\medskip\noindent
The notion of pseudofunctor $F:\mathcal{B}^{\text{op}} \to \mathsf{Cat}$
...
(This is very much like it happens in $\infty$-categories.)
It turns out that
$$
\text{Fibrations over $\mathcal{B}$}\xymatrix{\ar@{<~>}[r]&}
\text{pseudo functors }F:\mathcal{B}^{\text{op}}\to \mathsf{Cat}.
$$

\begin{definition}
\label{definition-fibred-in-groupoids}
A functor $p:\mathcal{E} \to \mathcal{B}$ is called a 
{\it fibration in groupoids} 
if it is a fibration and every fiber is a groupoid.
\end{definition}

\noindent
As I said in the beginning of this section,
I think this condition is equivalent to saying that
every morphism in the Étale space is cartesian
(this should be an easy exercise).

\begin{lemma}
\label{lemma-fibration-in-groupoids-iff}
$p:\mathcal{E} \to \mathcal{B}$ is a fibration in groupoids if and only if
every morphism in $\mathcal{E}$ is $p$-cartesian and any lifting problem
$$
\xymatrix{
& e\ar[d]\\
b\ar[r]& p(e)
...
From the hypotheses we get immediately that $p$ is a fibration.
To show that the fibers are groupoids we lift an arbitrary morphism $f$ 
and do lifting problem against itself.
\end{proof}

\begin{definition}
\label{definition-right-fibration-simplicial}

\end{definition}

\begin{lemma}
\label{lemma-fibration-nerves}
Let $X, Y$ be nerves of 1-categories.
Then $p:X \to Y$ is a right fibration if and only if
it is a fibration in groupoids.
\end{lemma}

\begin{proof}
The condition for right fibration for $n=0$ gives existence,
...
\begin{lemma}
\label{lemma-kan-fibration-left-right}
Kan fibration if and only if both left and right fibration.
\end{lemma}

\begin{lemma}
\label{lemma-kan-fibration-infty-groupoids}
$X$ is $\infty$-groupoid if and only if $X \to pt$ is Kan fibration,
if and only if it is left (or right) fibration.
\end{lemma}
