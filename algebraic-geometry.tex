...
\label{section-affine-algebraic-varieties}
...
\label{section-projective-varieties}
...
[x_0:\ldots:x_n]
=
[\lambda x_0:\ldots:\lambda x_n]
$$
for every $\lambda \in k^\times$.

\medskip\noindent
This means that ordinary polynomials do not
define functions on projective space. However,
if $f \in k[x_0,\ldots,x_n]$ is homogeneous of
degree $d$, then
$$
...
$$
Therefore the equation $f=0$ is well-defined
on $\mathbb{P}^n_k$.

\begin{definition}
\label{definition-projective-algebraic-set}
A {\it projective algebraic set} is a subset
$X \subset \mathbb{P}^n_k$ of the form
$$
X = V_+(I)
$$
where $I \subset k[x_0,\ldots,x_n]$ is a
homogeneous ideal and
$$
V_+(I)
=
\{p \in \mathbb{P}^n_k :
f(p)=0 \text{ for all homogeneous } f \in I\}.
$$
\end{definition}

\begin{definition}
\label{definition-projective-variety}
A {\it projective variety} is an irreducible
projective algebraic set.
\end{definition}

\noindent
The ideal of a projective algebraic set
$X \subset \mathbb{P}^n_k$ is
$$
I(X)
=
\{f \in k[x_0,\ldots,x_n] :
f \text{ is homogeneous and } f(p)=0
\text{ for all } p \in X\}.
$$

\medskip\noindent
Projective space is covered by affine charts.
For each $i$, define
$$
U_i =
\{[x_0:\ldots:x_n]\in \mathbb{P}^n_k :
x_i \neq 0\}.
$$
Then $U_i \cong \mathbb{A}^n_k$ by setting
$x_i=1$. For instance,
$$
U_0 \cong \mathbb{A}^n_k,
\qquad
[x_0:x_1:\ldots:x_n]
\longmapsto
\left(
\frac{x_1}{x_0},\ldots,\frac{x_n}{x_0}
\right).
$$
...
$$
\mathbb{P}^1_k=\{[x_0:x_1]\}.
$$
It is covered by two affine charts
$$
U_0=\{x_0\neq 0\},
\qquad
U_1=\{x_1\neq 0\}.
...
$t=x_1/x_0$, and on $U_1$ we use the
coordinate $s=x_0/x_1$. On the overlap,
$$
s = 1/t.
$$
\end{example}
...
\begin{example}
\label{example-projective-hypersurface}
If $F \in k[x_0,\ldots,x_n]$ is homogeneous,
then
$$
V_+(F) \subset \mathbb{P}^n_k
$$
is called a {\it projective hypersurface}.
For example,
$$
V_+(x_0^2+x_1^2-x_2^2)
\subset \mathbb{P}^2_k
$$
...
\medskip\noindent
The slogan is that projective varieties are
affine varieties together with points at
infinity, but the precise definition is:
projective varieties are cut out by
homogeneous equations in projective space.
...
$$
be the irrelevant ideal. Then
$$
\operatorname{Proj} S
$$
is the set of homogeneous prime ideals
$\mathfrak{p}\subset S$ such that
$$
...
\noindent
The closed sets of $\operatorname{Proj} S$ are
given by homogeneous ideals:
$$
V_+(I)
=
...
I\subset \mathfrak{p}\}.
$$
Equivalently, the basic open sets are
$$
D_+(f)
=
\{\mathfrak{p}\in \operatorname{Proj} S :
f\notin \mathfrak{p}\},
$$
where $f\in S$ is homogeneous.

\medskip\noindent
The affine pieces of $\operatorname{Proj} S$
are obtained by localizing and taking degree
zero:
$$
D_+(f) \cong \operatorname{Spec}(S_f)_0.
$$
Thus $\operatorname{Proj}$ is the projective
analogue of $\operatorname{Spec}$. 

\begin{example}
\label{example-projective-space-as-proj}
Projective space is
$$
\mathbb{P}^n_k
=
...
\section{Exercises: singularities, 
blow-up and normalization}
\label{section-exercises-sing-blow-up-norm}
