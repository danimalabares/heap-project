...
\noindent
A simple case of the following definition is when
$f$ is the inclusion and $X,W$ are both submanifolds of $Y$.

\begin{definition}
\label{definition-transversal-map}
Let $X,Y$ be smooth manifolds and $f \in C^\infty(X,Y)$.
Let $W \subset Y$ be a submanifold.
$f$ is {\it transversal} to $W$ if for all $x \in f^{-1}(W)$
$$
...
\noindent
This imposes restrictions on the dimensions of $X$ and $W$:
$$
\dim X+\dim W \geq \dim Y.
$$

This might also be thought of as a generalization of
the notion of regular value: if $W$ is a single point,
then the definition of $f$ being transversal to $Y$ becomes
$$
d_xf(T_xX)=T_yY,\qquad x \in f^{-1}(y)
$$
which is to say that $df$ is surjective everywhere.
