...
\label{theorem-bangart-hingston}
...
\begin{definition}
\label{definition-fiberwise-star-shaped}
...
\begin{definition}
\label{definition-h-inf-non-nilpotent}
...
\label{section-ambient-circle-widths}
...
pair, consider sweepouts by such pairs and
take a min-max of this distance. This gives
the number $\mathcal{S}_g(\Gamma)$.
...
\label{theorem-grove-shiohama-diameter}
...
\label{theorem-ams-width-critical-pair}
...
\label{theorem-amm-ambient-upper-bound}
...
\label{theorem-induced-sweepout-morse}
Let
$
...
\section{AI-summary of the paper}
\label{section-ai-summary-ivan-width-paper}
...
points of $\Gamma$. Organize these pairs into
sweepouts and define $\mathcal{S}_g(\Gamma)$
by minimizing the largest distance occurring
in a sweepout.

\medskip\noindent
{\bf Step 2. Relate it to critical points.}
The width is a min-max critical value of the
distance function. Since distance is not
smooth at the cut locus, the relevant notion
of critical point comes from the nonsmooth
critical-point theory of Grove--Shiohama.

\medskip\noindent
{\bf Step 3. Construct sweepouts.}
A basic tool is the induced-sweepout theorem.
A sufficiently generic Morse function on
$S^1$ produces a sweepout whose two points
always lie on the same level set. This turns
families of level curves in the ambient
manifold into useful sweepouts of the
embedded circle.

\medskip\noindent
{\bf Step 4. Compare with classical widths.}
For planar curves the new invariant is
compared with the classical width of a plane
curve. For a Riemannian two-sphere it is
compared with the Birkhoff min-max invariant:
$
\mathcal{S}_g(\Gamma)
\leq
\frac{1}{2}w_B(S^2,g).
$
The round sphere shows that this bound is
sharp.

\medskip\noindent
{\bf Step 5. Study equality and rigidity.}
The paper studies situations in which the
comparison inequalities are equalities or
nearly extremal. The point is that width data
coming from very simple objects can impose
strong restrictions on the ambient geometry.
This is where rigidity questions enter.

\medskip\noindent
{\bf Step 6. Pass from individual curves to
the whole manifold.}
Instead of fixing $\Gamma$, consider whether
all embedded circles in a complete
noncompact manifold have uniformly bounded
width. The paper characterizes this property
using the large-scale topology and geometry
of the manifold: there can be at most two
ends, and the manifold admits a map to a
finite graph whose fibers have uniformly
bounded diameter.

\medskip\noindent
This last condition says, in the language of
Urysohn width, that at large scale the
manifold behaves like a one-dimensional
space. Thus the overall progression is from
two points on a circle, to a min-max width,
to critical distance geometry, to comparison
with global invariants, and finally to
rigidity and large-scale information about
